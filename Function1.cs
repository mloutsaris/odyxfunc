...
namespace odyxfunc
{
    public class Function1
    {
        ...
        public async Task<IActionResult> AddLogoToPdfFromByteArray([HttpTrigger(AuthorizationLevel.Anonymous, "post")] HttpRequest req)
        {
            try
            {
                // Read the request body (expects a JSON object with the PDF byte array in base64 format)
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync();
                dynamic data = JsonConvert.DeserializeObject(requestBody);

                // Extract the base64-encoded PDF byte array
                string base64Pdf = data?.pdfBytes;
                ...
                // Call the helper method to add the logo and get the modified PDF byte array
                byte[] modifiedPdfBytes = await AddLogoToPdf_2(pdfBytes);
        ...
        private async Task<byte[]> AddLogoToPdf_2(byte[] pdfBytes)
        {
            try
            {
                // Convert byte array into MemoryStream for manipulation
                using (MemoryStream pdfStream = new MemoryStream(pdfBytes))
                {
                    ...
                    using (PdfDocumentProcessor pdfDocumentProcessor = new PdfDocumentProcessor())
                    {
                        ...
                        // Base64 strings for logos (replace with actual base64 strings)
                        ...
                        {
                            // Customize the coordinates as necessary
                        ...
                        {
                            // Save the modified PDF into the new MemoryStream
                            pdfDocumentProcessor.SaveDocument(modifiedPdfStream);
                            return modifiedPdfStream.ToArray(); // Return the modified PDF as byte array
                        }
                    }
                }
            }
